...
version=$(grep "^version:" "charts/bitwarden-eso-provider/Chart.yaml" | awk '{print $2}')
...
echo "Bumping version for bitwarden-eso-provider chart from $version to $major.$minor.$patch"
sed -i "s/^version:.*/version: ${major}.${minor}.${patch}/g" "charts/bitwarden-eso-provider/Chart.yaml"
